...
/datum/status_effect/his_grace/tick(seconds_between_ticks)
	...
	need_mob_update += owner.adjust_burn_loss(-grace_heal * seconds_between_ticks, updating_health = FALSE, forced = TRUE)
...
/datum/status_effect/fleshmend/tick(seconds_between_ticks)
	...
	need_mob_update += owner.adjust_burn_loss(-2 * seconds_between_ticks, updating_health = FALSE)
...
/datum/status_effect/hippocratic_oath/tick(seconds_between_ticks)
	if(owner.stat == DEAD)
		...
		if(iscarbon(owner))
			...
			need_mob_update += itemUser.adjust_burn_loss(-0.6 * seconds_between_ticks, updating_health = FALSE, forced = TRUE)
