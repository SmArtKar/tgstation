	...
	if(owner.maxHealth)
		...
		if(sleep_quality > 0)
			...
			if(health_ratio > 0.8) // only heals minor physical damage
				...
				need_mob_update += owner.adjust_burn_loss(-0.4 * sleep_quality * seconds_between_ticks, updating_health = FALSE, required_bodytype = BODYTYPE_ORGANIC)
...
/datum/status_effect/his_wrath/tick(seconds_between_ticks)
	...
	need_mob_update += owner.adjust_burn_loss(0.04 * seconds_between_ticks, updating_health = FALSE)
