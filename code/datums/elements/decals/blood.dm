...
/datum/element/decal/blood/Attach(datum/target, _icon, _icon_state, _dir, _plane, _layer, _alpha, _color, _smoothing, _cleanable=CLEAN_TYPE_BLOOD, _description, mutable_appearance/_pic, _filtered)
	if(!isitem(target))
		return ELEMENT_INCOMPATIBLE
	. = ..()
	RegisterSignal(target, COMSIG_ATOM_GET_EXAMINE_NAME, PROC_REF(get_examine_name), TRUE)
	RegisterSignal(target, COMSIG_ATOM_COLOR_UPDATED, PROC_REF(on_update_color), TRUE)
...
/datum/element/decal/blood/generate_appearance(_icon, _icon_state, _dir, _plane, _layer, _color, _alpha, _smoothing, source)
	...
	var/icon = as_item.icon
	var/icon_state = as_item.icon_state
	if(!icon || !icon_state)
		// It's something which takes on the look of other items, probably
		icon = as_item.icon
		icon_state = as_item.icon_state
	...
	var/mutable_appearance/blood_splatter = mutable_appearance('icons/effects/blood.dmi', "itemblood") //MA of the blood that we apply
	if(as_item.cached_color_filter)
		blood_splatter.appearance_flags |= KEEP_APART|RESET_COLOR
...
/datum/element/decal/blood/proc/on_update_color(obj/item/source, color_changed)
	SIGNAL_HANDLER
	if (!color_changed)
		return
	Detach(source)
