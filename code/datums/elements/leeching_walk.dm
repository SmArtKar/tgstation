...
/datum/element/leeching_walk/proc/on_life(mob/living/source, seconds_per_tick, times_fired)
	...
	need_mob_update += source.adjust_brute_loss(-3 * delta_time, updating_health = FALSE)
	need_mob_update += source.adjust_burn_loss(-3 * delta_time, updating_health = FALSE)
	need_mob_update += source.adjust_tox_loss(-3 * delta_time, updating_health = FALSE, forced = TRUE) // Slimes are people too
	need_mob_update += source.adjust_oxy_loss(-1.5 * delta_time, updating_health = FALSE)
	need_mob_update += source.adjust_stamina_loss(-10 * delta_time, updating_stamina = FALSE)
	if(need_mob_update)
		source.updatehealth()
