...
/datum/element/falling_hazard/proc/fall_onto_stuff(datum/source, turf/impacted_turf, levels)
	...
	var/target_head_armor = poor_target.run_armor_check(BRUTE, MELEE, MELEE_ATTACK, BODY_ZONE_HEAD, hit_by = source, source = source, silent = TRUE)
	...
	if(!crushes_people && target_head)
		poor_target.apply_damage(fall_damage * levels, def_zone = BODY_ZONE_HEAD, forced = TRUE, hit_by = source, source = source, wound_bonus = fall_wound_bonus)
	else
		poor_target.apply_damage(fall_damage * levels, forced = TRUE, hit_by = source, source = source, wound_bonus = fall_wound_bonus)
