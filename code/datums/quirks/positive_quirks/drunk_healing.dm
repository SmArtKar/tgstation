...
/datum/quirk/drunkhealing/process(seconds_per_tick)
	...
	switch(quirk_holder.get_drunk_amount())
		if (6 to 40)
			...
			need_mob_update += quirk_holder.adjust_burn_loss(-0.05 * seconds_per_tick, updating_health = FALSE, required_bodytype = BODYTYPE_ORGANIC)
		if (41 to 60)
			...
			need_mob_update += quirk_holder.adjust_burn_loss(-0.2 * seconds_per_tick, updating_health = FALSE, required_bodytype = BODYTYPE_ORGANIC)
		if (61 to INFINITY)
			...
			need_mob_update += quirk_holder.adjust_burn_loss(-0.4 * seconds_per_tick, updating_health = FALSE, required_bodytype = BODYTYPE_ORGANIC)
	if(need_mob_update)
