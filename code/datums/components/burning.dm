...
/datum/component/burning/process(seconds_per_tick)
	...
	if(atom_parent.resistance_flags & FIRE_PROOF)
		...
		return
	atom_parent.take_damage(10 * seconds_per_tick, BURN, FIRE, sound_effect = FALSE)
...
/datum/component/burning/proc/on_attack_hand(atom/source, mob/living/carbon/user)
	...
	user.apply_damage(5, BURN, FIRE, def_zone = user.get_active_hand()?.body_zone, source = parent, spread_damage = FALSE)
