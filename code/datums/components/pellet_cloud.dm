...
/datum/component/pellet_cloud/proc/finalize()
	var/obj/projectile/abstract_proj = new projectile_type(null)
	var/proj_name = abstract_proj.name
	...
	for(var/atom/target in targets_hit)
		...
		if(isbodypart(target))
			...
			if(!hit_part.owner) //only bother doing the thing if it was a limb just laying on the ground lol.
				...
				if(wound_info_by_part[hit_part] && (abstract_proj.damage_type == BRUTE || abstract_proj.damage_type == BURN)) // so a cloud of disablers that deal stamina don't inadvertently end up causing burn wounds)
					...
					var/wounding_type = (abstract_proj.damage_type == BRUTE) ? WOUND_BLUNT : WOUND_BURN // sharpness is handled in the wound rolling
					...
					var/mob/living/carbon/hit_carbon = target
					var/armor_factor = hit_carbon.getarmor(hit_part, abstract_proj.get_armor_flag())
					armor_factor = min(ARMOR_MAX_BLOCK, armor_factor) //cap damage reduction at 90%
					if(armor_factor > 0)
						if(abstract_proj.weak_against_armour && armor_factor >= 0)
							armor_factor *= ARMOR_WEAKENED_MULTIPLIER
						damage_dealt *= max(0, 1 - armor_factor*0.01)

					hit_part.painless_wound_roll(wounding_type, damage_dealt, w_bonus, bw_bonus, abstract_proj.sharpness)
