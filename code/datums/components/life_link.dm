...
/datum/component/life_link/proc/on_damage_adjusted(mob/living/our_mob, type, amount, forced)
	...
	if (forced)
		return
	amount *= our_mob.get_damage_mod(type) // Smartkar: o something about this and packages etc etc
	switch (type)
		if(BRUTE)
			host.adjust_brute_loss(amount, forced = TRUE)
		if(BURN)
			host.adjust_burn_loss(amount, forced = TRUE)
		if(TOX)
...
/datum/component/life_link/proc/on_limb_damage(mob/living/our_mob, limb, brute, burn)
	...
	if (burn != 0)
		host.adjust_burn_loss(burn, updating_health = FALSE)
	if (brute != 0 || burn != 0)
		host.updatehealth()
