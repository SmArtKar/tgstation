...
/datum/component/acid/proc/process_movable(atom/movable/target, seconds_per_tick)
	if(target.resistance_flags & ACID_PROOF)
		return
	target.take_damage(min(1 + round(sqrt(acid_power * acid_volume) * 0.3), MOVABLE_ACID_DAMAGE_MAX) * seconds_per_tick, BURN, ACID, sound_effect = FALSE)
...
/datum/component/acid/proc/on_attack_hand(atom/source, mob/living/carbon/user)
	...
	user.apply_damage(5, BURN, ACID, def_zone = user.get_active_hand()?.body_zone, source = parent)
