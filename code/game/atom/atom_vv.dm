...
/atom/vv_get_dropdown()
	...
	if(!ismovable(src))
		...
		if(curturf)
			options += "<button onclick=\"window.location.href='byond://?_src_=holder;[HrefToken()];adminplayerobservecoodjump=1;X=[curturf.x];Y=[curturf.y];Z=[curturf.z]';\">Jump To</button>"
	options += VV_DROPDOWN_OPTION(VV_HK_MODIFY_TRANSFORM, "Modify Transform")
	options += VV_DROPDOWN_OPTION(VV_HK_SPIN_ANIMATION, "SpinAnimation")
	options += VV_DROPDOWN_OPTION(VV_HK_STOP_ALL_ANIMATIONS, "Stop All Animations")
	options += VV_DROPDOWN_OPTION(VV_HK_SHOW_HIDDENPRINTS, "Show Hiddenprint log")
	options += VV_DROPDOWN_OPTION(VV_HK_ADD_REAGENT, "Add Reagent")
	options += VV_DROPDOWN_OPTION(VV_HK_TRIGGER_EMP, "EMP Pulse")
	options += VV_DROPDOWN_OPTION(VV_HK_TRIGGER_EXPLOSION, "Explosion")
	options += VV_DROPDOWN_OPTION(VV_HK_EDIT_FILTERS, "Edit Filters")
	options += VV_DROPDOWN_OPTION(VV_HK_EDIT_COLOR_MATRIX, "Edit Color as Matrix")
	options += VV_DROPDOWN_OPTION(VV_HK_TEST_MATRIXES, "Test Matrices")
	options += VV_DROPDOWN_OPTION(VV_HK_ADD_AI, "Add AI controller")
	options += VV_DROPDOWN_OPTION(VV_HK_ARMOR_MOD, "Modify Armor")
	if(greyscale_colors)
		options += VV_DROPDOWN_OPTION(VV_HK_MODIFY_GREYSCALE, "Modify greyscale colors")
	.[/atom] = options
