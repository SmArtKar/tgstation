...
/obj/item/tank/proc/handle_tolerances(seconds_per_tick)
	...
	if(temperature >= TANK_MELT_TEMPERATURE)
		...
		take_damage(max_integrity * temperature_damage_ratio * seconds_per_tick, BURN, FIRE, FALSE, NONE, attack_type = ENVIRONMENTAL_ATTACK)
		if(QDELETED(src))
			return TRUE
	...
	if(pressure >= TANK_LEAK_PRESSURE)
		...
		take_damage(max_integrity * pressure_damage_ratio * seconds_per_tick, BRUTE, BOMB, FALSE, NONE, attack_type = ENVIRONMENTAL_ATTACK)
		return TRUE
	return FALSE
