...
/obj/attacked_by(obj/item/attacking_item, mob/living/user)
	...
	var/damage = take_damage(total_force, attacking_item.damtype, attacking_item.get_damage_armor_type(), 1, get_dir(src, user))
...
/obj/handle_ricochet(obj/projectile/proj)
	...
	if(. && receive_ricochet_damage_coeff)
		take_damage(proj.damage * receive_ricochet_damage_coeff, proj.damage_type, proj.get_armor_flag(), 0, REVERSE_DIR(proj.dir), proj.armour_penetration) // pass along receive_ricochet_damage_coeff damage to the structure for the ricochet
