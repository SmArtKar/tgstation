...
/obj/structure/emergency_shield/modular/take_damage(datum/damage_package/damage, sound_effect = TRUE)
	. = ..()
	if(damage.damage_type == BRUTE || damage.damage_type == BURN)
		...
		shield_generator.shield_drain(.) // Can add or subtract a flat value to buff or nerf crowd damage
