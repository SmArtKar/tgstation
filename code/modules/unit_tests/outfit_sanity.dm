	...
	for (var/outfit_type in outfits_to_check)
		...
		if (outfit.backpack_contents || outfit.box)
			...
			for (var/path in backpack_contents)
				...
				for (var/_ in 1 to number)
					if (!H.equip_to_slot_or_del(new path(H), ITEM_SLOT_BACKPACK, TRUE, allow_locked = TRUE))
		...
		if (outfit.belt_contents)
			...
			for (var/path in belt_contents)
				...
				for (var/_ in 1 to number)
					if (!H.equip_to_slot_or_del(new path(H), ITEM_SLOT_BELTPACK, TRUE, allow_locked = TRUE))
