...
/datum/unit_test/novaflower_burn/Run()
	...
	TEST_ASSERT_EQUAL(botanist.get_burn_loss(), weapon.force, "The botanist picked up [weapon] with their bare hands, and took an incorrect amount of fire damage.")

	// Heal our attacker for easy comparison later
	botanist.adjust_burn_loss(-100)
	...
	TEST_ASSERT(botanist.get_burn_loss() <= 0, "The botanist took fire damage from [weapon], even though they were plant safe.")
	TEST_ASSERT_EQUAL(victim.get_burn_loss(), initial_force, "The target took an incorrect amount of fire damage after being hit with [weapon].")
