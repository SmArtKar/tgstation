...
/obj/item/organ/alien/plasmavessel/on_life(seconds_per_tick, times_fired)
	...
	if(locate(/obj/structure/alien/weeds) in owner.loc)
		if(owner.health >= owner.maxHealth)
			...
			owner.adjust_burn_loss(-heal_amt * delta_time_capped)
...
/obj/item/organ/stomach/alien/relaymove(mob/living/user, direction)
	...
	if(owner)
		...
		owner.apply_damage(impact / 2, BRUTE, MELEE, MELEE_ATTACK, BODY_ZONE_CHEST, hit_by = pokie, source = user, sharpness = SHARP_POINTY)
