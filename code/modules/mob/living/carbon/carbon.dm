...
/mob/living/carbon/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(iscarbon(hit_atom) && hit_atom != src)
		...
		if(blocked)
			...
			victim.apply_multiple_damages(brute = 10 + 5 * extra_speed, stamina = 10 + 10 * extra_speed, damage_flag = MELEE, attack_flags = LEAP_ATTACK, hit_by = src, spread_damage = FALSE, wound_bonus = extra_speed * 5, check_armor = TRUE)
			...
			victim.apply_damage(10 + 5 * extra_speed, BRUTE, MELEE, LEAP_ATTACK, hit_by = src, spread_damage = FALSE, wound_bonus = extra_speed * 5, check_armor = TRUE)
...
/mob/living/carbon/update_damage_hud()
	...
	var/hurtdamage = get_brute_loss() + get_burn_loss() + damageoverlaytemp
...
/mob/living/carbon/proc/can_defib()
	...
	if ((get_brute_loss() >= MAX_REVIVE_BRUTE_DAMAGE) || (get_burn_loss() >= MAX_REVIVE_FIRE_DAMAGE))
