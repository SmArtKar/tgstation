...
/datum/species/proc/handle_body(mob/living/carbon/human/species_human)
	...
	if(noggin && !(HAS_TRAIT(species_human, TRAIT_HUSK)))
		...
		if(!(NOEYESPRITES in species_traits))
			...
			if(!eye_organ)
				no_eyeslay = mutable_appearance('icons/mob/human_face.dmi', "eyes_missing_left", -BODY_LAYER)
				no_eyeslay.pixel_x += add_pixel_x
				no_eyeslay.pixel_y += add_pixel_y
				standing += no_eyeslay
				no_eyeslay = mutable_appearance('icons/mob/human_face.dmi', "eyes_missing_right", -BODY_LAYER)
				no_eyeslay.pixel_x += add_pixel_x
				no_eyeslay.pixel_y += add_pixel_y
				standing += no_eyeslay
