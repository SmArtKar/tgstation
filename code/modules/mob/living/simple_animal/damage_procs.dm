...
/mob/living/simple_animal/adjust_burn_loss(amount, updating_health = TRUE, forced = FALSE, required_bodytype)
	if(!can_adjust_burn_loss(amount, forced, required_bodytype))
		return 0
	if(forced)
		. = adjustHealth(amount * CONFIG_GET(number/damage_multiplier), updating_health, forced)
