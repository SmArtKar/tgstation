...
/obj/item/fish/dolphish/do_fish_process(seconds_per_tick)
	...
	switch(patience)
		if(0)
			...
			moc.apply_damage(force, BRUTE, MELEE, MELEE_ATTACK, moc.get_active_hand()?.body_zone, spread_damage = FALSE, hit_by = src, source = src, wound_bonus = wound_bonus, bare_wound_bonus = bare_wound_bonus, sharpness = get_sharpness(), check_armor = TRUE)
