...
/obj/effect/dummy/phased_mob/shadow/process(seconds_per_tick)
	...
	if(!QDELETED(jaunter) && isliving(jaunter)) //heal in the dark
		...
		living_jaunter.apply_multiple_heals(brute = (healing_rate * seconds_per_tick), burn = (healing_rate * seconds_per_tick), attack_flags = MAGIC_ATTACK, required_biotype = BODYTYPE_ORGANIC)
