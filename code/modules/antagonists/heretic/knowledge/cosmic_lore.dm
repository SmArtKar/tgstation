...
/datum/heretic_knowledge/blade_upgrade/cosmic/do_melee_effects(mob/living/source, mob/living/target, obj/item/melee/sickly_blade/blade)
	...
	need_mob_update += target.adjust_burn_loss(5, updating_health = FALSE)
	need_mob_update += target.adjust_organ_loss(pick(valid_organ_slots), 8)
	if(need_mob_update)
		target.updatehealth()
	...
	if(second_target_resolved)
		...
		need_mob_update += second_target_resolved.adjust_burn_loss(14, updating_health = FALSE)
		need_mob_update += second_target_resolved.adjust_organ_loss(pick(valid_organ_slots), 12)
		if(need_mob_update)
		...
		if(third_target_resolved)
			...
			need_mob_update = FALSE
			need_mob_update += third_target_resolved.adjust_burn_loss(28, updating_health = FALSE)
			need_mob_update += third_target_resolved.adjust_organ_loss(pick(valid_organ_slots), 14)
			if(need_mob_update)
