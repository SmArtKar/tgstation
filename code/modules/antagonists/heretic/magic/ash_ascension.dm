...
/datum/action/cooldown/spell/pointed/ash_beams/proc/fire_line(atom/source, list/turfs)
	...
	for(var/turf/beam_turf in turfs)
		if(isclosedturf(beam_turf))
			break

		for(var/mob/living/victim in beam_turf.contents)
		...
		new /obj/effect/hotspot(beam_turf)
		beam_turf.hotspot_expose(700,50,1)
		// deals damage to mechs
		for(var/obj/vehicle/sealed/mecha/mech in beam_turf.contents)
			if(mech in hit_list)
				continue
			hit_list += mech
			mech.take_damage(45, BURN, BLUNT, 1)
		sleep(0.15 SECONDS)
