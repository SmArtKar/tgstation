...
/obj/item/melee/blood_magic/proc/cast_spell(atom/target, mob/living/carbon/user)
	...
	if(health_cost)
		if(user.active_hand_index == 1)
			user.apply_damage(health_cost, BRUTE, null, MAGIC_ATTACK, BODY_ZONE_L_ARM, wound_bonus = CANT_WOUND)
		else
			user.apply_damage(health_cost, BRUTE, null, MAGIC_ATTACK, BODY_ZONE_R_ARM, wound_bonus = CANT_WOUND)
	if(uses <= 0)
		qdel(src)
	else if(source)
		source.desc = source.base_desc
...
/obj/item/melee/blood_magic/manipulator/proc/heal_cultist(mob/living/carbon/human/human_bloodbag, mob/living/carbon/human/user)
	...
	var/overall_damage = human_bloodbag.get_brute_loss() + human_bloodbag.get_burn_loss() + human_bloodbag.get_tox_loss() + human_bloodbag.get_oxy_loss()
	if(overall_damage == 0)
		if(blood_donor)
			return TRUE
		to_chat(user,span_cult("That cultist doesn't require healing!"))
	...
	need_mob_update += human_bloodbag.adjust_burn_loss(damage_healed * (human_bloodbag.get_burn_loss() / overall_damage), updating_health = FALSE)
