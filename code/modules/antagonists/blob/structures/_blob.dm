...
/obj/structure/blob/run_atom_armor(damage_amount, damage_type, damage_flag = NONE, attack_dir, attack_type = MELEE_ATTACK)
	switch(damage_type)
		if(BRUTE)
...
/obj/structure/blob/take_damage(damage_amount, damage_type = BRUTE, damage_flag = NONE, sound_effect = TRUE, attack_dir, armour_penetration = 0, ranged = FALSE)
	. = ..()
	if(. && atom_integrity > 0)
		update_appearance()
