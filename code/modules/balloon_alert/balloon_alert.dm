 ...
 */
/atom/proc/balloon_alert(mob/viewer, text, starting_x = 0, starting_y = 0)
	SHOULD_NOT_SLEEP(TRUE)

	INVOKE_ASYNC(src, PROC_REF(balloon_alert_perform), viewer, text, starting_x, starting_y)

/// Create balloon alerts (text that floats up) to everything within range.
/// Will only display to people who can see.
/atom/proc/balloon_alert_to_viewers(message, self_message, vision_distance = DEFAULT_MESSAGE_RANGE, list/ignored_mobs, starting_x = 0, starting_y = 0)
	SHOULD_NOT_SLEEP(TRUE)
	...
	for (var/mob/hearer in hearers)
		...
		balloon_alert(hearer, (hearer == src && self_message) || message, starting_x, starting_y)
...
/atom/proc/balloon_alert_perform(mob/viewer, text, starting_x, starting_y)
	...
	balloon_alert.maptext_x = (BALLOON_TEXT_WIDTH - bound_width) * -0.5 + starting_x
	balloon_alert.maptext_y = starting_y
