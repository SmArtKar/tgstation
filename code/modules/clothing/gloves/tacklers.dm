...
/obj/item/clothing/gloves/tackler/equipped(mob/user, slot)
	...
	if(slot & ITEM_SLOT_GLOVES)
		user.AddComponentFrom(REF(src), /datum/component/tackler, stamina_cost=tackle_stam_cost, base_knockdown = base_knockdown, range = tackle_range, speed = tackle_speed, skill_mod = skill_mod, min_distance = min_distance)

/obj/item/clothing/gloves/tackler/dropped(mob/user)
	. = ..()
	user.RemoveComponentSource(REF(src), /datum/component/tackler)
