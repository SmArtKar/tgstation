...
/obj/item/clothing/head/helmet/monkey_sentience/examine(mob/user)
	...
	. += result.show_message("BLOOD RAGE")
	. += result.show_message("BRAIN DEATH")
	. += result.show_message("PRIMAL GENE ACTIVATION")
	. += result.show_message("GENETIC MAKEUP MASS SUSCEPTIBILITY")
	. += result.show_message("Warranty voided if helmet is placed after more than <b>two</b> mind magnification failures.")
	. += result.show_message("Ask your CMO if mind magnification is right for you!")
