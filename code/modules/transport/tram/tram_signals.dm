...
/obj/machinery/transport/crossing_signal/examine(mob/user)
	...
	switch(operating_status)
		if(TRANSPORT_REMOTE_WARNING)
			. += result.show_message("The orange [EXAMINE_HINT("remote warning")] light is on.")
			. += result.show_message("The status display reads: Check track sensor.")
		if(TRANSPORT_REMOTE_FAULT)
			. += result.show_message("The blue [EXAMINE_HINT("telecoms failure")] light is on.")
			. += result.show_message("The status display reads: Check telecommunications network.")
		if(TRANSPORT_LOCAL_FAULT)
			. += result.show_message("The red [EXAMINE_HINT("local fault")] light is on.")
			. += result.show_message("The status display reads: Repair required.")
	switch(dir)
		if(NORTH, SOUTH)
			. += result.show_message("The tram configuration display shows EAST/WEST.")
		if(EAST, WEST)
			. += result.show_message("The tram configuration display shows NORTH/SOUTH.")
...
/obj/machinery/transport/guideway_sensor/examine(mob/user)
	...
	switch(operating_status)
		if(TRANSPORT_REMOTE_WARNING)
			. += result.show_message("The orange [EXAMINE_HINT("remote warning")] light is on.")
			. += result.show_message("The status display reads: Check paired sensor.")
		if(TRANSPORT_REMOTE_FAULT)
			. += result.show_message("The blue [EXAMINE_HINT("remote fault")] light is on.")
			. += result.show_message("The status display reads: Paired sensor not found.")
		if(TRANSPORT_LOCAL_FAULT)
			. += result.show_message("The red [EXAMINE_HINT("local fault")] light is on.")
			. += result.show_message("The status display reads: Repair required.")
