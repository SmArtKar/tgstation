...
/obj/item/clothing/head/hooded/alloy_armor/proc/resonate(mob/user)
	...
	for(var/mob/living/simple_animal/target in view(9, get_turf(user)))
		if(faction_check(user.faction, target.faction))
			continue
		target.Stun(2 SECONDS + 0.15 SECONDS * (9 - get_dist(user, target)))
		target.adjustBruteLoss(75 + 5 * (9 - get_dist(user, target)))
		...
		if(ishostile(target))
			...
			hostile.ranged_cooldown += 2 SECONDS + 0.15 SECONDS * (9 - get_dist(user, target))
