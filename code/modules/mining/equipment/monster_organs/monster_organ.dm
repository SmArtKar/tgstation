...
/obj/item/organ/monster_core/examine(mob/user)
	...
	estimated_time_left = round(estimated_time_left, 1 MINUTES)
	switch(estimated_time_left)
		if(4 MINUTES)
			. += result.show_message("It's fresh and still pulsating with the last vestiges of life.")
		if(3 MINUTES)
			. += result.show_message("It still looks pretty fresh.")
		if(2 MINUTES)
			. += result.show_message("It's not as fresh as could be.")
		if(1 MINUTES)
			. += result.show_message("Signs of decay are starting to set in. It might not be good for much longer.")
		if(0 SECONDS to 1 MINUTES)
			. += result.show_message("Signs of decay have set in, but it still looks alive. It's probably about to become unusable really quickly.")
