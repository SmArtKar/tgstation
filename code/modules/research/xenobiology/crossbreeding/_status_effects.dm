...
/datum/status_effect/slime_clone_decay/tick(seconds_between_ticks)
	...
	need_mob_update += owner.adjust_burn_loss(1, updating_health = FALSE)
	if(need_mob_update)
		owner.updatehealth()
	owner.color = "#007BA7"
...
/datum/status_effect/stabilized/purple/tick(seconds_between_ticks)
	...
	if(owner.get_burn_loss() > 0)
		need_mob_update += owner.adjust_burn_loss(-0.2, updating_health = FALSE)
		healed_last_tick = TRUE
