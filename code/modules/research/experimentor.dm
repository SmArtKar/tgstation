...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			for(var/mob/living/victim in oview(1, src))
				victim.deal_damage(15, BRUTE, pick(BODY_ZONE_HEAD,BODY_ZONE_CHEST,BODY_ZONE_CHEST), MELEE, attack_type = ENVIRONMENTAL_ATTACK)
				investigate_log("Experimentor dealt minor brute to [key_name(victim)].", INVESTIGATE_EXPERIMENTOR)
			ejectItem(TRUE)
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			throwSmoke(loc)
			for(var/mob/living/victim in oview(1, src))
				victim.deal_damage(5, BURN, pick(BODY_ZONE_HEAD,BODY_ZONE_CHEST,BODY_ZONE_CHEST), FIRE, attack_type = ENVIRONMENTAL_ATTACK)
				investigate_log("Experimentor has dealt minor burn damage to [key_name(victim)]", INVESTIGATE_EXPERIMENTOR)
			ejectItem()
	////////////////////////////////////////////////////////////////////////////////////////////////
