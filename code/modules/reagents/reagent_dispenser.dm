...
/obj/structure/reagent_dispensers/take_damage(damage_amount, damage_type = BRUTE, damage_flag = NONE, sound_effect = TRUE, attack_dir, armour_penetration = 0, ranged = FALSE)
	. = ..()
	if(. && atom_integrity > 0)
		if(tank_volume && damage_flag == LASER)
			boom()
