...
/datum/reagent/consumable/ethanol/cuba_libre/on_mob_life(mob/living/carbon/cubano, seconds_per_tick, times_fired)
	...
	if(cubano.mind && cubano.mind.has_antag_datum(/datum/antagonist/rev)) //Cuba Libre, the traditional drink of revolutions! Heals revolutionaries.
		...
		need_mob_update += cubano.adjust_burn_loss(-1 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
...
/datum/reagent/consumable/ethanol/manly_dorf/on_mob_life(mob/living/carbon/dwarf, seconds_per_tick, times_fired)
	...
	if(dorf_mode)
		...
		need_mob_update += dwarf.adjust_burn_loss(-2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/bananahonk/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if((liver && HAS_TRAIT(liver, TRAIT_COMEDY_METABOLISM)) || is_simian(drinker))
		if(drinker.apply_multiple_heals(brute = 1 * REM * seconds_per_tick, burn = 1 * REM * seconds_per_tick, attack_flags = REAGENT_ATTACK, spread_damage = FALSE, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/hearty_punch/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.health <= 0)
		...
		need_mob_update += drinker.adjust_burn_loss(-3 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
...
/datum/reagent/consumable/ethanol/quintuple_sec/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(liver && HAS_TRAIT(liver, TRAIT_LAW_ENFORCEMENT_METABOLISM))
		var/need_mob_update
		need_mob_update = drinker.apply_multiple_heals(brute = 2 * REM * seconds_per_tick, burn = 2 * REM * seconds_per_tick, stamina = 5 * REM * seconds_per_tick, required_bodytype = affected_bodytype, spread_damage = FALSE, attack_flags = REAGENT_ATTACK, updating_health = FALSE)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/bastion_bourbon/on_mob_metabolize(mob/living/drinker)
	...
	need_mob_update += drinker.adjust_burn_loss(-heal_amt, updating_health = FALSE, required_bodytype = affected_bodytype)
...
/datum/reagent/consumable/ethanol/bastion_bourbon/on_mob_life(mob/living/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.health > 0)
		...
		need_mob_update += drinker.adjust_burn_loss(-1 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
...
/datum/reagent/consumable/ethanol/between_the_sheets/on_mob_life(mob/living/drinker, seconds_per_tick, times_fired)
	...
	var/need_mob_update
	if(drinker.get_brute_loss() && drinker.get_burn_loss()) //If you are damaged by both types, slightly increased healing but it only heals one. The more the merrier wink wink.
		if(prob(50))
			...
			need_mob_update = drinker.adjust_burn_loss(-0.25 * REM * seconds_per_tick, updating_health = FALSE,  required_bodytype = affected_bodytype)
	...
	else if(drinker.get_burn_loss())
		need_mob_update = drinker.adjust_burn_loss(-0.2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/trappist/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.mind?.holy_role)
		if(drinker.adjust_burn_loss(-2.5 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/kortara/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.get_brute_loss() && SPT_PROB(10, seconds_per_tick))
		if(drinker.apply_healing(1 * REM * seconds_per_tick, BRUTE, null, REAGENT_ATTACK, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
