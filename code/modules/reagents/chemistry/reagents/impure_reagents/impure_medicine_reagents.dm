...
/datum/reagent/impurity/healing/medicine_failure/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	switch(pick)
		...
		if("burn")
			need_mob_update += affected_mob.adjust_burn_loss(-0.5, updating_health = FALSE, required_bodytype = affected_bodytype)
		if("tox")
...
/datum/reagent/inverse/libitoil/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	affected_mob.adjust_organ_loss(ORGAN_SLOT_LIVER, 0.1 * REM * delta_time)
...
/datum/reagent/inverse/aiuri/on_mob_life(mob/living/carbon/owner, delta_time, times_fired)
	owner.adjust_organ_loss(ORGAN_SLOT_EYES, 0.1 * REM * delta_time)
...
/datum/reagent/inverse/penthrite/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	var/need_mob_update
	need_mob_update = affected_mob.adjust_brute_loss(5 * (1-creation_purity) * seconds_per_tick, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_organ_loss(ORGAN_SLOT_HEART, (1 + (1-creation_purity)) * seconds_per_tick, required_organ_flag = affected_organ_flags)
