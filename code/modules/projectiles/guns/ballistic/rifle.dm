...
/obj/item/gun/ballistic/rifle/boltaction/pipegun/examine_more(mob/user)
	...
	. += result.show_message("It's said that the first slaying committed on a Nanotrasen space station was by an assistant.")
	. += result.show_message("That this act, done by toolbox, maybe spear, was what consigned their kind to a life of destitution, rejection and violence.")
	. += result.show_message("They carry the weight of this act visibly; the grey jumpsuit. Breathing deeply filtered air. And with bloodsoaked yellow hands clenched into fists. Eyes, sharp and waiting. Hunters in the dark.")
	. += result.show_message("Eventually, these killing spirits sought to stake a claim on the metal tombs they were trapped within. Rejecting their status. Determined to be something more.")
	. += result.show_message("This weapon is one such tool. And it is a grim one indeed. Wrought from scrap, pulled from the station's walls and floors and the very nails holding it together.")
	. += result.show_message("It is a symbol that the true masters of this place are not those who merely inhabit it. But the one willing to twist it towards a killing intent.")
