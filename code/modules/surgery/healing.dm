...
/datum/surgery_step/heal/initiate(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, try_to_fail = FALSE)
	if(!..())
		return
	while((brutehealing && target.get_brute_loss()) || (burnhealing && target.get_burn_loss()))
		if(!..())
			break
...
/datum/surgery_step/heal/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	if(target.stat == DEAD) //dead patients get way less additional heal from the damage they have.
		...
		burn_healed += round((target.get_burn_loss() * (burn_multiplier * 0.2)),0.1)
		dead_patient = TRUE
	else
		brute_healed += round((target.get_brute_loss() * brute_multiplier),0.1)
		burn_healed += round((target.get_burn_loss() * burn_multiplier),0.1)
		dead_patient = FALSE
...
/datum/surgery_step/heal/burn/get_progress(mob/user, mob/living/carbon/target, brute_healed, burn_healed)
	if(!burn_healed)
		return
	var/estimated_remaining_steps = target.get_burn_loss() / burn_healed
	var/progress_text

	if(locate(/obj/item/healthanalyzer) in user.held_items)
		progress_text = ". Remaining burn: <font color='#ff9933'>[target.get_burn_loss()]</font>"
	else
		switch(estimated_remaining_steps)
			if(-INFINITY to 1)
				return
...
/datum/surgery_step/heal/combo/get_progress(mob/user, mob/living/carbon/target, brute_healed, burn_healed)
	...
	if(burn_healed > 0)
		estimated_remaining_steps = max(estimated_remaining_steps, (target.get_burn_loss() / burn_healed)) // whichever is higher between brute or burn steps
