...
/obj/item/organ/liver/examine(mob/user)
	...
	if(HAS_TRAIT(src, TRAIT_LAW_ENFORCEMENT_METABOLISM))
		. += result.show_message("Fatty deposits and sprinkle residue, imply that this is the liver of someone in <em>security</em>.")
	if(HAS_TRAIT(src, TRAIT_CULINARY_METABOLISM))
		. += result.show_message("The high iron content and slight smell of garlic, implies that this is the liver of a <em>cook</em>.")
	if(HAS_TRAIT(src, TRAIT_COMEDY_METABOLISM))
		. += result.show_message("A smell of bananas, a slippery sheen and [span_clown("honking")] when depressed, implies that this is the liver of a <em>clown</em>.")
	if(HAS_TRAIT(src, TRAIT_MEDICAL_METABOLISM))
		. += result.show_message("Marks of stress and a faint whiff of medicinal alcohol, imply that this is the liver of a <em>medical worker</em>.")
	if(HAS_TRAIT(src, TRAIT_ENGINEER_METABOLISM))
		. += result.show_message("Signs of radiation exposure and space adaption, implies that this is the liver of an <em>engineer</em>.")
	if(HAS_TRAIT(src, TRAIT_BALLMER_SCIENTIST))
		. += result.show_message("Strange glowing residues, sprinklings of congealed solid plasma, and what seem to be tumors indicate this is the radiated liver of a <em>scientist</em>.")
	if(HAS_TRAIT(src, TRAIT_MAINTENANCE_METABOLISM))
		. += result.show_message("A half-digested rat's tail (somehow), disgusting sludge, and the faint smell of Grey Bull imply this is what remains of an <em>assistant</em>'s liver.")
	if(HAS_TRAIT(src, TRAIT_CORONER_METABOLISM))
		. += result.show_message("An aroma of pickles and sea water, along with being remarkably well-preserved, imply this is what remains of a <em>coroner</em>'s liver.")
	if(HAS_TRAIT(src, TRAIT_HUMAN_AI_METABOLISM))
		. += result.show_message("The liver appears barely human and entirely self-sufficient, implying this is what remains of a <em>human AI</em>'s liver.")

	// royal trumps pretender royal
	if(HAS_TRAIT(src, TRAIT_ROYAL_METABOLISM))
		. += result.show_message("A rich diet of luxury food, suppleness from soft beds, implies that this is the liver of a <em>head of staff</em>.")
	else if(HAS_TRAIT(src, TRAIT_PRETENDER_ROYAL_METABOLISM))
		. += result.show_message("A diet of imitation caviar, and signs of insomnia, implies that this is the liver of <em>someone who wants to be a head of staff</em>.")
